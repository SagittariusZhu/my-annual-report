...
export const fetchData = async token => {
    ...
    const create_repos = repos.filter(item => {
        return is2019(item.created_at);
    })

    const modified_repo = repos.filter(item => {
        return is2019(item.updated_at);
    })

    return {
        created_at: dealTime(user.created_at),
        first_repo: create_repos[create_repos.length - 1].name,
        repo_count: create_repos.length,
        modified_repo_count: modified_repo.length,
